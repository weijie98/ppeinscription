...
$mdpCorrection = sha1($_POST["password"]);
...
$reqLoginCorrection = $baseDeDonnees->prepare('SELECT * FROM eleves WHERE username=:username AND password=:password(SELECT * FROM entreprise WHERE username=:username AND password=:password)(
SELECT * FROM professeur WHERE username=:username AND password=:password)');
...
else if ($nombreLignesRetournees == 1)
{
